...
const HEARTBEAT_TIMEOUT = 15000;  // 15秒内没收到pong则认为断开
...
const server = http.createServer((req, res) => {
    ...
    } else if (urlPath === '/wu-online.html') {
        filePath = path.join(__dirname, 'wu-online.html');
    } else if (urlPath === '/wu.html') {
...
// 心跳相关变量
const clientPingTime = new WeakMap();
...
function startHeartbeat(ws) {
    ...
    const timer = setInterval(() => {
        if (ws.readyState === WebSocket.OPEN) {
            ...
            // 设置超时，超过时间没收到pong则断开
            setTimeout(() => {
                if (ws.readyState === WebSocket.OPEN && !clientPingTime.get(ws)) {
...
wss.on('connection', (ws) => {
    ...
    ws.on('pong', () => {
        // 收到客户端的pong响应，清除超时标记
        clientPingTime.set(ws, Date.now());
    });
        ...
        if (ws.roomId) {
            ...
            if (room) {
                ...
                if (player) {
                    ...
                    console.log(`Player ${player.name} disconnected, waiting for reconnect...`);

                    // 5秒后如果还没重连，则判定真正离开
                    setTimeout(() => {
                        const currentRoom = rooms.get(ws.roomId);
                        if (currentRoom) {
                            const reconnectedPlayer = currentRoom.players.find(p => p.ws === ws);
    ...
    ws.on('message', (data) => {
        try {
            ...
            if (msg.type === 'reconnect') {
                ...
                if (room) {
                    const player = room.players.find(p => p.color === msg.playerColor && p.disconnected);
                    if (player && Date.now() - player.disconnectTime < 5000) {
                        // 重连成功
                        player.ws = ws;
                        player.disconnected = false;
                        ws.roomId = msg.roomId;        // 关键：设置房间ID
                        ws.playerColor = msg.playerColor; // 关键：设置玩家颜色
